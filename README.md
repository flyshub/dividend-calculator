...
- **股东总回报率（TSR）**：估值不变假设下的一年总回报估计 = (ROE − 股息率×PB) + 股息率，详情页分析区块 + 选股器列双呈现
- **分红可持续性评估**：股利支付率 / 利润趋势 / 特别分红 / 连续分红年数 / 未分配利润覆盖 / 净现比 六维判据 + 银行专项模块
- **股价与股息率走势图**：近十年双轴图，含高股息区间与除权日标记
...
> PE 与 ROE 均取百分数值（如 PE=13.5、ROE=15.2），基础/修正版直接相除；PB 版 ROE 为百分数，`PB/ROE²×100` 等价于代码实现 `pb/(ROE/100)²/100`。
...
> N 因子是市赚率发明人（雪球 ericwarn丁宁）自认的「经验公式」，非理论推导、无学术出处：50% 基准取自贵州茅台多年平均股利支付率（作者以茅台为「赚真钱」估值锚，支付率低于 50% 视为「赚假钱」，N 相应放大）。作者自评修正市赚率「只在最近几年才有效」，随国企加大分红可能失效——仅作参考，不作唯一决策依据。

估值区间：≤0.5 低估 / 0.5-1.0 合理偏低 / 1.0-3.0 合理 / >3.0 高估

> 阈值基于 PR 历史回测（2016-2024 沪深300，[回测报告](docs/BACKTEST_REPORT.md)）：PR≤1（低估+合理偏低）超额 +4%+，PR>3 显著跑输（-5pct+），最低 PR 组有 alpha（+6.2%，行业中性化后仍成立）。市赚率用于「避贵」+「选低估值」。
...
> 假设 PB 一年内不变：`ROE − 股息率×PB` 为留存收益推动的每股净资产增速（此时即价格回报），加股息率为现金分红回报，合计为持有一年的总回报估计。

> 口径：股息率取真实股息率（税前、完整财年总额法）；ROE 周期股取 5 年 ROE 中位数（缺失回退最新年报，对齐 PB-市赚率先例）；无分红时公式退化为 ROE。估值变化（PB 抬升/回落）不在估计范围内。

> TSR > 15% 时页面标注「高回报」徽标（详情页股票卡 + 选股器行）——为约定阈值、非回测结论，不构成估值评级。
...
- **606 Python 单元测试 + 115 JS 单元测试**：财年推断、市赚率公式、股东总回报率、可持续性评分、数据源注入、双端对齐、选股器
...
本工具可作为**初筛过滤器**：选出的标的仍需深入阅读年报、关注行业景气度与公司治理，不可完全依赖量化结果做投资决策。工具输出的是「数学上便宜」的股票，但「便宜」≠「值得买」，请结合个人风险偏好与资产配置需求。
