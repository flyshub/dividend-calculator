...
估值区间：≤0.5 低估 / 0.5-1.0 合理偏低 / 1.0-3.0 合理 / >3.0 高估（阈值基于 PR 历史回测，见 docs/BACKTEST_REPORT.md；市赚率用于避贵 + 选低估值——PR≤1 有超额，PR>3 跑输）

> ⚠️ 回测数字以 docs/BACKTEST_REPORT.md 为准：页面回测提示、估值提示语、docstring 中的超额/跑输数字（+4%/~0%/-5.3% 等）散布在多处，改回测结论时**先更新报告，再同步所有引用点**。
